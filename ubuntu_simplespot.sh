...
COMPLETED_STEPS+=("FreeRADIUS REST module enabled and configured")
...
# Configure FreeRADIUS default site (Redis configurations removed)
...
mysql -e "DELETE FROM mysql.user WHERE User='';" 2>/dev/null || echo "Could not remove anonymous user"
mysql -e "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1');" 2>/dev/null || echo "Could not remove remote root user"
mysql -e "DROP DATABASE IF EXISTS radius;" 2>/dev/null || echo "Could not remove radius database"
mysql -e "DELETE FROM mysql.db WHERE Db='radius' OR Db='radius\\_%';" 2>/dev/null || echo "Could not remove radius database"
